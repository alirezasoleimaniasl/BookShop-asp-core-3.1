    ...
    public class ApplicationIdentityErrorDescriber:IdentityErrorDescriber
    {
        ...
        public override IdentityError PasswordRequiresNonAlphanumeric()
            => new IdentityError
            {
                ...
                Description = "کلمه عبور باید شامل کاراکترهای غیرعددی و غیرحرفی مانند(!،@،#،$،%،^،&،*،) باشد"
            };
        public override IdentityError PasswordRequiresDigit()
            => new IdentityError
            {
                Code = nameof(PasswordRequiresDigit),
                Description = "کلمه عبور باید حداقل یک عدد (0-9) باشد"
            };
        ...
        public override IdentityError InvalidUserName(string userName)
            => new IdentityError
            {
                ...
                Description = "نام کاربری باید شامل کاراکترهای (0-9) , (a-z,A-Z) باشد"
            };
        public override IdentityError DuplicateEmail(string email)
            => new IdentityError
            {
                Code=nameof(DuplicateEmail),
                Description = $"شما قبلا با این ایمیل{email} ثبت نام کرده اید"
            };
        public override IdentityError DuplicateRoleName(string role)
            => new IdentityError
            {
                Code = nameof(DuplicateRoleName),
                Description = $"شما قبلا نقش {role} را اضافه کرده اید"
            };
    }
}
